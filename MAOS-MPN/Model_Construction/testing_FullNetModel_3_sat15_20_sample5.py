...
# Modification Date:20240613 在10sat——1800task和2000task上进行测试
# Modification Date:20240613 对110sat的网络的文件夹：
#       F:\PythonTestNew\Data_timewindows\MAOS_Standard_Data_202405\test1_FinalData中的数据进行测试
# Programmer: LZ
import numpy as np
import torch
...
from NetModel_FullAdvanced_1 import MultiAgentModel, TrainDataset
...
if __name__ == '__main__':
    ...
    actor_param = torch.load(path_parent + '\\NetModel_save\MultiPointerNetwork_{}sat_0.pt'.format(num_sat))
    ...
    # 0 加载数据
    num_sample = 5
    for num_task in range(1000, 2001, 500):
        path_load = 'F:\PythonTestNew\Data_timewindows\MAOS_Standard_Data_202405\\test1_FinalData' \
                    '\DatasetTesting_{}sat_{}task_5.pt'.format(num_sat, num_task)


        test_data = torch.load(path_load)
        test_data = TrainDataset(test_data)
        ...
        path = path_parent + '\TestingResults\\MPN_FullNetmodel_0'
        # os.mkdir(path)
        fp = open(path_parent + '\TestingResults\\Testing Log.txt', 'a', encoding='utf-8')
        fp.write('\n' + time_str + ' MPN_FullNetmodel_{}sat_{}task_sample{} 测试结果保存路径：'.
                 format(num_sat, num_task, num_sample))
        fp.close()
        del time_str
        ...
        path_save = path + '\MPN_FullNetModelResult_sat{}_task{}_sample{}.pt'.\
            format(num_sat, num_task, num_sample)
