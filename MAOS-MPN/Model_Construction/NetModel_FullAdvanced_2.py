                ...
                for bi in ub_set:   # 逐个样本处理，计算单步调度结果
                    ...
                    if loss_vtw_tasks.size(0) > 0:     # 存在损失观测机会的任务
                        ...
                        if not dt_state.all():  # 表示有假,即没有后续观测机会了
                            ...
                            batch_prob[bi, deleted_tasks] = 1  # 删除任务的概率为1
...
class TrainDataset(Dataset):
    def __init__(self, var):
        self.require, self.wn, self.vtw, self.mo, self.norm_req, self.norm_gw, self.norm_lw = var
        self.size = self.require.size(0)  # 样本个数

    def __len__(self):
        return self.size

    def __getitem__(self, idx):
        return self.require[idx], self.wn[idx], self.vtw[idx], self.mo[idx], self.norm_req[idx], self.norm_gw[idx], \
               self.norm_lw[idx]
...
if __name__ == "__main__":
    ...
    tli = 0
    for num_task in range(800, 1401, 200):
        ...
        require, wn, vtw, mo, norm_req, norm_gw, norm_lw = torch.load(path_load)
        train_data[tli] = TrainDataset((require[: num_sample], wn[: num_sample], vtw[: num_sample], mo[: num_sample],
                               norm_req[: num_sample], norm_gw[: num_sample], norm_lw[: num_sample]))
        tli += 1
        del require, wn, vtw, mo, norm_req, norm_gw, norm_lw  # 加载后即删除，节省内存空间
    ...
    train_parser.add_argument('--epochs', default=5, type=int, help='完整训练的轮次')
    ...
    step_sum = TrainArgs.epochs * 1280 * 4 / TrainArgs.batch_size
    step_size = 10
    actor_scheduler = Adaptive_CosAnneal_WarmRestart(actor_optim, total_steps=step_sum, step_size=step_size,
                                                     deacy_rate=0.01, min_lr=0.0005, min_lr_second=0.0008)
    actor.train()
    ...
    for epoch in range(TrainArgs.epochs):
        for sample_type in range(4):
            ...
            list_eval = []
            train_loader = DataLoader(train_data[sample_type], TrainArgs.batch_size, shuffle=True, drop_last=True)
            for batch_idx, batch in enumerate(train_loader):
                ...
                count_times += 1
            # 每一类数据计算完保存一下
            path_save = path + '\StageResult_sat{}_task{}_epoch{}.pt'.format(num_sat, int(sample_type*20+800), epoch)
            torch.save((list_eval, results, results_delete, time.time()- start_time0), path_save)
            path_save = path + '\StageNetModel_sat{}_task{}_epoch{}.pt'.format(num_sat, int(sample_type * 20 + 800),
                                                                             epoch)
