...
from NetModel_FullAdvanced_1 import TrainDataset, trend_mean, Adaptive_CosAnneal_WarmRestart
...
if __name__ == '__main__':
    ...
    path = path_parent + '\TrainingResults\\MultiAgentModel_Greedy_ALR_' + now
    os.mkdir(path)
    fp = open(path_parent + '\TrainingResults\\Training Log.txt', 'a', encoding='utf-8')
    fp.write('\n' + time_str + ' MultiAgentModel_Greedy_ALR 训练结果保存路径：' + path)
    fp.close()
    del time_str
    print('训练结果保存路径：', path)
    ...
    train_parser.add_argument('--epochs', default=1, type=int, help='完整训练的轮次')
    ...
    actor = MultiAgentModel_Greedy(ModelArgs.dim_require, ModelArgs.dim_vtw, ModelArgs.dim_hidden, ModelArgs.num_heads,
