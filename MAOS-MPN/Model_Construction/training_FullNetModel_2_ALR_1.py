...
# Modification Date: 20240616 训练15颗卫星和20颗的网络，并且删除任务不计概率
# Programmer: LZ
...
from NetModel_FullAdvanced_2 import MultiAgentModel, TrainDataset
from NetModel_FullAdvanced_1 import trend_mean, Adaptive_CosAnneal_WarmRestart
...
if __name__ == '__main__':
    ...
    require = deepcopy(require[:960])
    wn = deepcopy(wn[:960])
    vtw = deepcopy(vtw[:960])
    mo = deepcopy(mo[:960])
    norm_req = deepcopy(norm_req[:960])
    norm_gw = deepcopy(norm_gw[:960])
    norm_lw = deepcopy(norm_lw[:960])
    gc.collect()
    train_data = TrainDataset((require, wn, vtw, mo, norm_req, norm_gw, norm_lw))
    ...
    path = path_parent + '\TrainingResults\\FullNetmodel2_ALR_{}_'.format(num_sat) + now
    os.mkdir(path)
    fp = open(path_parent + '\TrainingResults\\Training Log.txt', 'a', encoding='utf-8')
    fp.write('\n' + time_str + ' FullNetmodel2_ALR 训练结果保存路径：' + path)
    fp.close()
    del time_str
    print('训练结果保存路径：', path)
    ...
    model_parser.add_argument('--num_agent', default=num_sat, type=int, help='最大卫星数对应最多agent数')
    ModelArgs = model_parser.parse_args()

    # 2.3 训练参数
    train_parser = argparse.ArgumentParser(description='Parameters of training')
    train_parser.add_argument('--batch_size', default=8, type=int, help='批大小')
    ...
    step_sum = TrainArgs.epochs * 960 / TrainArgs.batch_size
    step_size = 10
    ...
    torch.save((actor_rewards, actor_losses, use_time), path_save)
    print('用时：', use_time)
